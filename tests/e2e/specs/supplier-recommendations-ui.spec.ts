...
test.describe('Modul Supplier Recommendations UI - E2E Tests', () => {
     ...
     test('Positif - LOAD supplier catalog page (index)', async ({ page }) => {
          /**
     ...
     test('Positif - LOAD product comparison page', async ({ page }) => {
          /**
           * Given: User authenticated
     ...
     test('Positif - DISPLAY product list dari seeder', async ({ page }) => {
          /**
          ...
          // Assert - Products should be visible (from seeder)
     ...
     test('Positif - SELECT product menampilkan comparison table', async ({ page }) => {
          /**
           * Given: Products page loaded
           * When: Click on a product dari sidebar
           * Then: Comparison table rendered dengan supplier data
           */
          ...
          // Assert - Comparison table rendered
     ...
     test('Positif - FILTER comparison by sort (cheapest)', async ({ page }) => {
          /**
     ...
     test('Positif - FILTER comparison by sort (closest)', async ({ page }) => {
          /**
           * Given: Product comparison table loaded
     ...
     test('Positif - FILTER comparison by stock (instock only)', async ({ page }) => {
          /**
          ...
          // All visible rows should have stock > 0
          const stockCells = page.locator('tbody tr td span:has-text("unit")');
          const count = await stockCells.count();

          if (count > 0) {
               // At least one row should be visible with stock info
               await expect(stockCells.first()).toBeVisible();
          }
     });

     test('Positif - SAW ranking column displayed when AHP configured', async ({ page }) => {
          /**
           * Given: User dengan valid AHP weights (CR ≤ 0.1)
          ...
          // If AHP configured, these should be visible
     ...
     test('Positif - WARNING displayed when AHP not configured', async ({ page }) => {
          /**
     ...
     test('Positif - CLICK "Buka Toko" navigates to supplier detail', async ({ page }) => {
          /**
     ...
     test('Positif - SEARCH products by name', async ({ page }) => {
          /**
           * Given: Products page
     ...
     test('Positif - LOAD supplier detail page', async ({ page }) => {
          /**
           * Given: Valid supplier ID dari seeder (assume ID 1 exists)
           * When: Navigate to /spk-suppliers/1
           * Then: Supplier detail page rendered dengan complete info
           */
     ...
     test('Positif - DISPLAY supplier metrics (score, rating)', async ({ page }) => {
          /**
           * Given: Supplier detail page loaded
           * When: Check metrics section
     ...
     test('Positif - DISPLAY supplier inventories (products)', async ({ page }) => {
          /**
     ...
     test('Negatif - LOAD supplier detail dengan ID tidak exist (404)', async ({ page }) => {
          /**
           * Given: Non-existent supplier ID
           * When: Navigate to /spk-suppliers/99999
           * Then: Show 404 error or redirect
           */

          // Act
          const response = await page.goto('/spk-suppliers/99999');

          // Assert - Should be 404 or error page
     ...
     test('Integration - Full user journey: Catalog → Product Comparison → Supplier Detail', async ({ page }) => {
          /**
     ...
     test('Integration - Search and filter combination works', async ({ page }) => {
          /**
