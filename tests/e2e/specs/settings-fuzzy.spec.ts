...
test.describe.serial('Modul Dashboard Konfigurasi Logic Fuzzy - E2E Pjwb QA', () => {
    ...
    test('Positif - Reset konfigurasi fuzzy ke default', async ({ page }) => {
    ...
    test('Positif - Tab Sumber Data memuat konten dengan benar', async ({ page }) => {
        await fuzzyPage.clickSourcesTab();
    ...
    test('Positif - Membuat rule dengan kondisi AND lengkap', async ({ page }) => {
        await fuzzyPage.clickRulesTab();
        await page.waitForTimeout(500);
        await fuzzyPage.addRuleButton.click();
        await page.waitForTimeout(1000);
        await page.evaluate(() => {
            const forms = document.querySelectorAll('form');
            const form = Array.from(forms).find(f =>
                f.innerHTML.includes('Simpan') && f.innerHTML.includes('name="operator"')
            );
            if (!form) return 'rule form not found';
            const g = form.querySelector('[name="group"]') as HTMLSelectElement;
            if (g) g.value = 'lingkungan';
            const op = form.querySelector('[name="operator"]') as HTMLSelectElement;
            if (op) op.value = 'AND';
            const d = form.querySelector('[name="diagnosis"]') as HTMLInputElement;
            if (d) d.value = 'qa test rule';
            form.submit();
            return 'ok';
        });
