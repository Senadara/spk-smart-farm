@php
    $priorityMeta = [
        'urgent' => ['label' => 'Urgent', 'badge' => 'bg-rose-50 text-rose-700 border-rose-200', 'edge' => 'border-l-rose-400'],
        'high' => ['label' => 'Tinggi', 'badge' => 'bg-amber-50 text-amber-700 border-amber-200', 'edge' => 'border-l-amber-400'],
        'medium' => ['label' => 'Sedang', 'badge' => 'bg-sky-50 text-sky-700 border-sky-200', 'edge' => 'border-l-sky-400'],
        'low' => ['label' => 'Rendah', 'badge' => 'bg-slate-50 text-slate-600 border-slate-200', 'edge' => 'border-l-slate-300'],
    ];
...
<div class="group rounded-xl border border-l-4 border-slate-100 {{ $meta['edge'] }} bg-white p-3.5 shadow-sm transition-all hover:border-slate-200 hover:shadow-md">
        ...
        <div class="flex items-center gap-1 opacity-100 transition-opacity sm:opacity-0 sm:group-hover:opacity-100">
            @if($canWork && $task->status === 'todo')
                <form method="POST" action="{{ route('spk.tasks.status', $task->id) }}" class="inline">
                    ...
                    <button type="submit" title="Mulai Kerjakan" class="flex h-6 w-6 items-center justify-center rounded-md bg-sky-50 text-sky-600 transition hover:bg-sky-100">
            ...
            @if($canWork && $task->status === 'in_progress')
                <button @click="openReport(@js($task->id), @js($task->title))" title="Kirim Laporan" class="flex h-6 w-6 items-center justify-center rounded-md bg-emerald-50 text-emerald-600 transition hover:bg-emerald-100">
            ...
            <a href="{{ route('spk.tasks.show', $task->id) }}" title="Detail" class="flex h-6 w-6 items-center justify-center rounded-md bg-slate-50 text-slate-500 transition hover:bg-slate-100">
