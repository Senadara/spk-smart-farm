...
<div class="max-w-full space-y-5">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2 px-3 py-2 bg-white border border-[var(--color-gray-200)] rounded-lg text-sm text-[var(--color-gray-600)]">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {{-- Blok Kebun Aktif --}}
        <x-perkebunan.stat-card
            label="Blok Kebun Aktif"
            :value="$kebunStats['aktif']"
            :subtitle="'dari ' . $kebunStats['total_blok'] . ' total blok'"
        ...
        <x-perkebunan.stat-card
            label="Total Tanaman"
            :value="number_format($kebunStats['total_tanaman'])"
            subtitle="tanaman melon aktif"
        ...
        <x-perkebunan.stat-card
            ...
            :value="$evaluasiStatusLabels[$evStatus] ?? 'Draft'"
            :subtitle="Str::limit($evaluasiTerbaru['nama_sesi'] ?? '-', 50)"
        ...
        <x-perkebunan.stat-card
            ...
            :subtitle="$alertSummary['critical'] . ' kritis, ' . $alertSummary['warning'] . ' peringatan'"
                ...
                <div class="p-5">
                    ...
                    <dl class="space-y-3">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-0.5 sm:gap-2">
                            ...
                            <dd class="text-sm font-medium text-[var(--color-gray-900)]">{{ $evaluasiTerbaru['nama_sesi'] ?? '-' }}</dd>
                        </div>
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-0.5 sm:gap-2">
                            <dt class="text-sm text-[var(--color-gray-500)]">Status</dt>
                            <dd>
                                @php
                                    $statusBadgeColor = ['selesai' => 'green', 'berlangsung' => 'amber', 'draft' => 'blue'][$evStatus] ?? 'blue';
                                @endphp
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-0.5 sm:gap-2">
                            ...
                            <dd class="text-sm font-medium text-[var(--color-gray-900)]">{{ \Carbon\Carbon::parse($evaluasiTerbaru['tanggal'] ?? now())->translatedFormat('d F Y') }}</dd>
                        </div>
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-0.5 sm:gap-2">
                            <dt class="text-sm text-[var(--color-gray-500)]">Jumlah Alternatif</dt>
                            <dd class="text-sm font-medium text-[var(--color-gray-900)]">{{ $evaluasiTerbaru['jumlah_alternatif'] ?? '-' }} blok kebun</dd>
                        </div>
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-0.5 sm:gap-2">
                            <dt class="text-sm text-[var(--color-gray-500)]">Dinilai Oleh</dt>
                            <dd class="text-sm font-medium text-[var(--color-gray-900)]">{{ $evaluasiTerbaru['dinilai_oleh'] ?? '-' }}</dd>
                        </div>
                    </dl>
                    ...
                    <div class="mt-4 pt-3 border-t border-[var(--color-gray-100)]">
                        <a href="#" class="inline-flex items-center gap-1 text-xs font-medium text-[var(--color-primary)] hover:underline">
        ...
        <div class="p-5">
                        ...
                        <tbody>
                            @foreach($rankingTerbaru['items'] as $item)
                                @php
                                    ...
                                    $keputusanConfig = [
                                        'disetujui'         => ['color' => 'green',  'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/>', 'label' => 'Disetujui'],
                                        'ditunda'           => ['color' => 'amber',  'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M15.75 5.25v13.5m-7.5-13.5v13.5"/>', 'label' => 'Ditunda'],
                                        'ditolak'           => ['color' => 'red',    'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>', 'label' => 'Ditolak'],
                                        'belum_divalidasi'  => ['color' => 'blue',   'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z"/>', 'label' => 'Belum Divalidasi'],
                                    ];
                                    $kConfig = $keputusanConfig[$item['status_keputusan']] ?? $keputusanConfig['belum_divalidasi'];
                                @endphp
                                <tr class="{{ $isFirst ? 'bg-[var(--color-primary-lighter)]/30 border-l-4 border-l-[var(--color-primary)]' : '' }}">
                                    <td class="text-center font-bold text-[var(--color-gray-900)]">{{ $item['peringkat'] }}</td>
                                    <td class="font-medium text-[var(--color-gray-900)]">{{ $item['blok'] }}</td>
                                    <td>
                                        <div class="flex items-center gap-3">
                                            <span class="text-sm font-semibold text-[var(--color-gray-900)] w-16" title="Skor Preferensi TOPSIS">{{ number_format($item['skor'], 4) }}</span>
                                            <div class="flex-1 h-1.5 bg-[var(--color-gray-100)] rounded-full overflow-hidden">
                                                <div class="h-full bg-[var(--color-primary)] rounded-full transition-all" style="width: {{ $skorPersen }}%"></div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="text-center">
                                        <x-badge :color="$kConfig['color']">
                                            <span class="inline-flex items-center gap-1">
                                                <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">{!! $kConfig['icon'] !!}</svg>
                                                {{ $kConfig['label'] }}
                                            </span>
                                        </x-badge>
                                    </td>
            ...
            @else
                ...
                <div class="text-center py-8">
                    ...
                    <p class="text-xs text-[var(--color-gray-300)] mt-1">Ranking akan muncul setelah evaluasi SPK pertama selesai</p>
                </div>
            @endif
        </div>
    </x-card>
        ...
        @else
            ...
            <x-card>
                <div class="text-center py-8">
                    ...
                    <p class="text-xs text-[var(--color-gray-300)] mt-1">Data sensor akan muncul setelah integrasi IoT Antares aktif</p>
                </div>
            </x-card>
        @endif
    </div>
