...
<div {{ $attributes->merge(['class' => 'card']) }}>
    ...
    <div class="flex items-start gap-3">
        ...
        <div class="flex items-center justify-center w-10 h-10 rounded-xl shrink-0 {{ $iconBg }}">
            <svg class="w-5 h-5 {{ $iconColor }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                {!! $icon !!}
            </svg>
        </div>
        ...
        <div class="flex-1 min-w-0">
            <p class="text-[11px] font-medium uppercase tracking-wider text-[var(--color-gray-400)] mb-0.5">{{ $label }}</p>
            <p class="text-xl font-bold text-[var(--color-gray-900)] leading-tight">{{ $value }}</p>
            ...
            @if($trend)
                ...
                @endphp
                <span class="inline-flex items-center gap-1 px-1.5 py-0.5 text-xs font-semibold rounded-full mt-1.5 {{ $badgeClass }}">
