    ...
    <div class="space-y-6 p-6">
        ...
        <nav class="flex items-center gap-2 text-sm text-gray-600">
            ...
            <a href="{{ route('spk-melon.sesi-penilaian.show', $sesi->id) }}"
                class="hover:text-green-600">{{ $sesi->namaSesi }}</a>
            <span>/</span>
        ...
        {{-- Panduan penggunaan --}}
        <details class="rounded-xl border border-gray-200 bg-white shadow-sm p-5"
            {{ !$isReadOnly && empty($existingMatrix) ? 'open' : '' }}>
            ...
            <div class="mt-3 space-y-2 text-sm text-gray-700">
                ...
                <p>2. Pilih nilai Skala Saaty (1-9) yang menggambarkan tingkat kepentingan kriteria <em>baris</em>
                    dibandingkan kriteria <em>kolom</em>:</p>
            ...
            <form method="POST" action="{{ route('spk-melon.sesi-penilaian.perbandingan.update', $sesi->id) }}"
                ...
                <div class="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm">
                    <table class="min-w-full text-sm">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="border-b border-gray-200 px-4 py-3 text-left font-semibold text-gray-700">
                                    Kriteria</th>
                                @foreach ($kriteriaList as $kCol)
                                    <th class="border-b border-gray-200 px-3 py-3 text-center font-semibold text-gray-700">
                                        ...
                                        <div class="text-[10px] font-normal text-gray-500 truncate max-w-[140px]"
                                            title="{{ $kCol->nama }}">
                            ...
                            @foreach ($kriteriaList as $i => $kRow)
                                <tr class="border-b border-gray-100 last:border-none">
                                    <td class="bg-gray-50 px-4 py-3 font-medium text-gray-900">
                                        ...
                                        <div class="text-[10px] font-normal text-gray-500 truncate max-w-[160px]"
                                            title="{{ $kRow->nama }}">
                                    ...
                                    @elseif ($i > $j) bg-gray-50 @endif">
                                            ...
                                            @elseif ($i < $j)
                                                ...
                                                <select name="perbandingan[{{ $kRow->id }}::{{ $kCol->id }}]"
                ...
                @unless ($isReadOnly)
                    <div class="mt-6 flex items-center justify-end gap-3">
                        <a href="{{ route('spk-melon.sesi-penilaian.show', $sesi->id) }}"
                            class="flex items-center gap-2 rounded-lg bg-red-500 hover:bg-red-600 px-5 py-2.5 text-sm font-medium text-white shadow-sm transition-all">
                            <i data-lucide="x" class="w-4 h-4"></i>
                            Batal
                        </a>
                        <button type="submit"
